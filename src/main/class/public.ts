...
class Public {
   ...
   */
  get_language_path() {
    return path.resolve(utils.getExtraResourcesDir(), 'languages')
  }
  ...
  async httpRequest(
    ...
    try {
      ...
      const requestOptions: any = {
        method: method,
        url: url,
        headers: options.headers || { 'User-Agent': 'AiKnowledgeBase/' + this.version() },
        timeout: timeout,
  ...
  get_resource_path(): string {
    try {
      return utils.getExtraResourcesDir()
    } catch (e) {
