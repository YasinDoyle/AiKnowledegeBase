...
export class ChatService {
  ...
  mergeHistory(chatHistory: ChatHistory[]): ChatHistory[] {
    ...
    for (let history of chatHistory) {
      ...
      let index = mergedHistory.findIndex(
        (item) => item.compare_id == history.compare_id && item.role == 'user',
      )
      ...
      // 合并assistant
      index = mergedHistory.findIndex(
        (item) => item.compare_id == history.compare_id && item.role == 'assistant',
      )
  ...
  save_chat_history(
    uuid: string,
    history: ChatHistory,
    historyRes: ChatHistory,
    contextLength: number,
    regenerate_id: string | undefined,
  ): void {
    // 为历史记录生成唯一标识符
    history.id = pub.uuid()
    // 计算历史记录的 tokens 数量
