...
// 读取供应商模型列表的通用函数
async function readSupplierModels(
  fileName: string,
  contextLengthFunc: (model: string) => number,
): Promise<{ [key: string]: ModelInfo[] }> {
    ...
    try {
      ...
      for (const model of models) {
        const modelInfo: ModelInfo = {
          title:
            model.title ||
            `${supplierConfig.supplierTitle || supplierConfig.supplierName}/${model.modelName}`,
