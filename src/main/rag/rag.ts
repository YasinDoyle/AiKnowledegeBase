...
const getTemplate = (
  ...
  DEEPSEEK_PROMPT_TPL: string
  DEEPSEEK_SYSTEM_PROMPT_TPL: string
  OTHER_PROMPT_TPL: string
  OTHER_SYSTEM_PROMPT_TPL: string
  ...
  let TEMPLATES_LANG = [
    ...
    pub.lang(
      '并非检索结果的所有内容都与用户的问题密切相关，你需要结合问题，对检索结果进行甄别、筛选。',
    ),
    agentInfo
      ? ''
      : pub.lang(
          '对于列举类的问题（如列举所有航班信息），尽量将答案控制在10个要点以内，并告诉用户可以查看检索来源、获得完整信息。优先提供信息完整、最相关的列举项；如非必要，不要主动告诉用户检索结果未提供的内容。',
        ),
    agentInfo
      ? ''
      : pub.lang(
          '对于创作类的问题（如写论文），你需要解读并概括用户的题目要求，选择合适的格式，充分利用检索结果并抽取重要信息，生成符合用户要求、极具思想深度、富有创造力与专业性的答案。你的创作篇幅需要尽可能延长，对于每一个要点的论述要推测用户的意图，给出尽可能多角度的回答要点，且务必信息量大、论述详尽。',
        ),
    agentInfo
      ? ''
      : pub.lang(
  ...
  let OTHER_SYSTEM_PROMPT_TPL_LANG = [
    ...
    pub.lang('在回答时，请注意以下几点'),
    pub.lang(
      '根据提供的检索结果生成信息丰富且与用户查询相关的回答，如果知识库检索结果中有图片引用信息，可根据需要引用这些图片来强化内容结构。',
    ),
    pub.lang('当前日期和时间为'),
    pub.lang('用户所在地区为'),
    pub.lang('不要提及检索结果的具体排名。'),
    pub.lang(
      '并非检索结果的所有内容都与用户的问题密切相关，你需要结合用户的意图，对检索结果进行甄别、筛选。',
    ),
    agentInfo
      ? ''
      : pub.lang(
          '对于列举类的问题（如列举所有航班信息），尽量将答案控制在10个要点以内，并告诉用户可以查看检索来源、获得完整信息。优先提供信息完整、最相关的列举项',
        ),
    agentInfo
      ? ''
      : pub.lang(
...
const generateDeepSeekPrompt = (
  ...
  const search_results = searchResultList
    .map(
      (result, idx) =>
...
${doc_files_str}`
  ...
  const userPrompt = DEEPSEEK_PROMPT_TPL.replace('{search_results}', search_results)
...
const generateOtherPrompt = (
  ...
  let doc_files_str = doc_files
    .map((doc_file, idx) => `<doc source="${doc_file}" id="${idx + 1}">${doc_file}</doc>`)
    .join('\n')
...
${doc_files_str}
  ...
  const systemPrompt = OTHER_SYSTEM_PROMPT_TPL.replace('{search_results}', search_results)
    .replace('{current_date_time}', currentDateTime)
    .replace('{user_location}', userLocation)
    .replace('{doc_files}', doc_files_str)
...
export const getDefaultPrompt = (
  ...
  if (model.indexOf('deepseek') !== -1) {
    userPrompt = DEEPSEEK_SYSTEM_PROMPT_TPL.replace('{current_date_time}', currentDateTime)
      .replace('{user_location}', userLocation)
      .replace('{question}', query)
      .replace('{search_results}', '')
  } else {
    systemPrompt = OTHER_SYSTEM_PROMPT_TPL.replace('{current_date_time}', currentDateTime)
      .replace('{user_location}', userLocation)
      .replace('{question}', query)
      .replace('{search_results}', '')
  }
...
export class Rag {
  ...
  public async createDocTable(tableName: string): Promise<boolean> {
    ...
    let ok = await LanceDBManager.createTableAt(
      tableName,
      [
        {
          doc_id: '0',
      ...
      for (let i = 0; i < searchResultList.length; i++) {
        ...
        if (currentLength + docLength > maxLength) {
          if (currentLength == maxLength) {
            ...
            searchResultList[i].content = searchResultList[i].content.substring(
              0,
  ...
  public async reindexRag(argName: string): Promise<boolean> {
    await LanceDBManager.updateRecord(argName, {
      where: `doc_aag='${argName}'`,
      values: { is_parsed: 0 },
    })
    return true
  }
