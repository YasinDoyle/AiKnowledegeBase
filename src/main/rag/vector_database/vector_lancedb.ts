...
export class LanceDBManager {
  ...
  public static async optimizeAllTable() {
    try {
      ...
      pub.write_file(
        tipFile,
        `optimizedTables: ${optimizedTables.join(',')}\ntime: ${endTime - startTime}s`,
      )
    } catch (e) {
      logger.error('优化表失败', e)
    } finally {
   ...
   */
  private static async getEmbedding(
    supplierName: string,
    model: string,
    text: string,
  ): Promise<number[]> {
    ...
    try {
      ...
      if (!res.embedding || res.embedding.length !== this.DIMENSION) {
        if (!res.embedding) {
          throw new Error(
            `嵌入维度错误: 期望 ${this.DIMENSION}, 实际 ${res.embedding ? res.embedding.length : 0}, 模型: ${model}, 文本: ${text}`,
          )
        }
        // 不足的维度以0填充
        if (res.embedding && res.embedding.length < this.DIMENSION) {
   ...
   * @returns boolean
   */
  public static async createTableAt(
    tableName: string,
    tableStruct: any[],
    indexKeys: any[],
  ): Promise<boolean> {
  ...
  public static async hybridSearchByNew(
    tableName: string,
    ragInfo: {
    ...
    const embedding = await this.getEmbedding(
      ragInfo.supplierName,
      ragInfo.embeddingModel,
      queryText,
    )
   ...
   */
  private static async optimizeDocumentContent(
    results: any[],
    docNameMap: Map<string, any>,
  ): Promise<any[]> {
    // 计算文档切片的总长度
        ...
        for (let i = 0; i < uniqueDocIds.length; i += batchSize) {
          ...
          if (orConditions) {
            ...
            results.forEach((item: any) => {
              ...
              result.set(item.doc_id, {
                doc_name: item.doc_name,
                doc_file: docFile,
                doc: pub.read_file(docFile),
              })
            })
          }
        }
  ...
  public static async deleteDocument(tableName: string, docId: string): Promise<number> {
    ...
    try {
      ...
      const exists =
        (await tableObj.query().where(where).select(['docId']).limit(1).toArray()).length > 0
      ...
      const scoredResults = results.map((result) => {
        ...
        for (const keyword of processedKeywords) {
          ...
          if (keywordIndex !== -1) {
            ...
            const afterChar =
              keywordIndex + keyword.length < matchDoc.length
            ...
            if (processedKeywords.length > 1) {
              const nextKeyword = processedKeywords.find(
                (k) => k !== keyword && matchDoc.includes(k),
              )
              if (nextKeyword) {
        ...
        if (matchMode === 'all') {
          // 所有关键词必须匹配才能获得分数
          finalScore =
            matchedKeywords === processedKeywords.length
